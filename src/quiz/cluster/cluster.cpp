...
void proximity(const std::vector<std::vector<float>>& points, int id, std::vector<int> & cluster,
					std::map<int, bool>& visited, KdTree*& tree, float distanceTol)
{
	if(!visited[id])
	{
		visited[id] = true;
		...
		{
			proximity(points, neighbor, cluster, visited, tree, distanceTol);
		}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i=0; i<points.size(); i++)
	{
		if(!visited[i])
		{
			...
			proximity(points, i, cluster, visited, tree, distanceTol);
